...
namespace Catch
{
    ...
    {
        SonarQubeReporter(ReporterConfig const& config) : CumulativeReporterBase(config), xml(config.stream())
        {
        ...
        void writeGroup(TestGroupNode const& groupNode)
        {
            ...
            for (auto const& child : groupNode.children) testsPerFile[child->value.testInfo.lineInfo.file].push_back(child);

            for (auto const& kv : testsPerFile) writeTestFile(kv.first.c_str(), kv.second);
        }
        ...
        {
            for (auto const& assertion : sectionNode.assertions) writeAssertion(assertion, okToFail);
        }
